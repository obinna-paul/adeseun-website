 ...
 */
export default async function CheckoutPage({ params }: { params: Promise<{ bookId: string }> }) {
  const { bookId } = await params;
  const book = BOOKS.find((b) => b.id === bookId);
  if (!book) notFound();
      ...
      <div className="mx-auto grid max-w-frame gap-12 lg:grid-cols-[minmax(0,240px)_1fr] lg:gap-16">
        <div className="mx-auto w-full max-w-[200px] lg:mx-0 lg:max-w-none">
          <div className="aspect-[2/3] w-full">
            <BookCover book={book} className="h-full shadow-elevation-card" />
          </div>
          <div className="mt-6">
            ...
            <p className="mt-3 font-display text-2xl text-text">{formatNaira(book.price)}</p>
          </div>
        </div>
